...
.directive('ngGallery', ['$document', '$timeout', '$q', '$templateCache', function($document, $timeout, $q, $templateCache) {
	...
	'    <img style="background-image: url({{i.thumb}});background-position: center center;" class="{{ thumbClass }}" ng-click="openGallery($index)" />' +
	'  </div>' +
	'</div>' +
	...
	'  <span class="info-text">Pic # {{ index + 1 }}/{{ images.length }} <br>Title: {{ title }}<br>Date :{{ date }}<br> Description :{{ description }}</span>' +
	...
	return {
		...
		link: function (scope, element, attrs) {
			...
			var showImage = function (i) {
				...
				scope.date = scope.images[i].date || '';
			};
