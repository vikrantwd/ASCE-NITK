...
var eventList = function(callback){
	db.EventsOppCol.find({"subpageOf" : "mainpage","page":"opportunities"},function(err,data){
		events = data[0];
...
router.get('/', function(req, res, next) {
	async.series([footerlist,eventList,relatedEvents],function(err,results){
		if(!err){
			console.log(footer);
			res.render('opportunities',{ParentPage :'Main',footer: footer,events: events,relEvents:relEvents});
		}
	});
});
